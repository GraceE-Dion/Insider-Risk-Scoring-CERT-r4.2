...
#
# Computes cumulative feature snapshots for tracked users across all 17
# observation months and scores each snapshot to produce a risk trajectory.
# Demonstrates the "low-and-slow" behavioral ramp-up pattern characteristic
# of insider threats prior to incident completion.
#
# Key findings:
#   All 5 malicious users show gradual score escalation crossing the Critical
#   threshold (0.75) between mid-2010 and early 2011.
#   Benign users remain near zero throughout except for one temporary spike
#   that self-corrects — the false positive behavioral signature.
#
# Outputs: Risk_Trajectory_Malicious.png
#          Risk_Trajectory_Benign.png
# Note: This cell takes approximately 2-3 minutes to run.
# =============================================================================
...
for month in months:
    ...
    for df_merge, cols in [
        ...
        (df_m, None),
        (ef,   None),
        (ah_r[['user','after_hours_ratio']], None),
        (udpd[['user','unique_devices_per_day']], None),
        (lbr[['user','logon_burst_ratio']], None),
        (ebr[['user','email_burst_ratio']], None)
    ]:
        snap = snap.merge(df_merge, on='user', how='left')
