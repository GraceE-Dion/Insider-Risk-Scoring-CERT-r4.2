...
# A user whose score is rising rapidly is more actionable than a user with
# the same absolute score but stable trend, even if both are below a
# critical threshold.
#
# Velocity threshold: +0.10 per month
# High velocity events detected: 18 (15 malicious, 3 benign)
#
# Key finding: Malicious users show velocity spikes concentrated in mid-2010
# (July-October) — the behavioral acceleration phase prior to incident
# completion. Benign velocity is tightly clustered near zero.
#
...
for user in tracked_users:
    ...
    for i in range(1, len(ud)):
        ...
        velocity_records.append({
            'user': user, 'month': ud['month'].iloc[i],
...
# ── Plot 1: Velocity over time — malicious ────────────────────────────────────
